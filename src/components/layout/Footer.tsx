...
const footerLinks = {
  ...
  support: [
    ...
    { href: '/account/tickets', label: 'Submit a Ticket' },
    { href: '/contact', label: 'Contact Us' },
  ],
  services: [
    { href: '/quote', label: 'Get an Instant Quote' },
    { href: '/booking', label: 'Rate & Ship' },
    { href: '/track', label: 'Track Shipment' },
    { href: '/how-it-works', label: 'How It Works' },
  ],
};
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-2 lg:grid-cols-[220px_1fr_1fr_1fr_240px] gap-x-6 gap-y-8 lg:gap-12 py-12 lg:py-20">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {footerLinks.company.map(({ href, label }) => (
                <li key={label}>
                  <Link href={href} className="text-sm text-gray-600 hover:text-brand-orange transition">{label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="mt-4 space-y-1">
              <a href="tel:6477017778" className="block text-sm text-gray-600 hover:text-brand-orange transition">📞 647-701-7778</a>
              <a href="tel:18882001845" className="block text-sm text-gray-600 hover:text-brand-orange transition">📞 1-888-200-1845</a>
              <a href="mailto:support@pilotcourier.com" className="block text-sm text-gray-600 hover:text-brand-orange transition">✉ support@pilotcourier.com</a>
            </div>
          </div>
        ...
        <div className="border-t border-gray-200 py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <p className="text-sm text-gray-500">
            © {new Date().getFullYear()} Pilot Courier Canada. All rights reserved.
          </p>
          <div className="flex flex-wrap items-center gap-3">
            {[
              ...
              <Link key={label} href={href} className="text-sm text-gray-500 hover:text-brand-orange transition">{label}</Link>
            ))}
          </div>
        </div>
      </div>
